Write-Host "Working Directory: $((Get-Item -Path '.\' -Verbose).FullName)" -ForegroundColor Magenta
...
For ($i=0; $i -lt $filesArr.Length; $i++) {
    ...
    write-host "Private settings count: $privateSettingsFound" -ForegroundColor Magenta
    write-host "Private connection settings count: $privateConnectionStringsFound" -ForegroundColor Magenta
